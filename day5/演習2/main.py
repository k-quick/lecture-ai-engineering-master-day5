...
class DataLoader:
    ...
    @staticmethod
    def load_titanic_data(path="day5\演習2\data\Titanic.csv"):
        """Titanicデータセットを読み込む"""
        if path:
...
def test_inference_speed_and_accuracy():
    ...
    assert metrics['inference_time'] < 1.0, \
        f"推論時間が遅すぎます: {metrics['inference_time']:.4f}秒 (期待値: 1.0秒未満)"

    # 精度のテスト（0.75以上であること）
    assert metrics['accuracy'] >= 0.75, \
        f"モデルの精度が低すぎます: {metrics['accuracy']:.4f} (期待値: 0.75以上)"

    # 推論時間と精度の表示
    print(f"テスト成功 - 推論時間: {metrics['inference_time']:.4f}秒, 精度: {metrics['accuracy']:.4f}")
